...
export default function AIChatbox({ apiKey, onRequestKey }) {
    const formatText = (text) => {
        if (!text) return '';
        // 1. Nếu có thẻ <think>...</think>, loại bỏ nó để tránh giao diện hiển thị rác trước khi format
        let cleanText = text.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();

        // 2. Escape HTML an toàn
        let safeText = cleanText.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');

        // 3. Format in đậm và xuống dòng
        safeText = safeText.replace(/\*\*(.*?)\*\*/g, '<strong style="color:#00f5ff">$1</strong>');
    ...
    const sendMessage = async () => {
        ...
        try {
            // Bỏ qua tin nhắn chào mừng (role: model) đầu tiên vì Gemini API yêu cầu lịch sử phải bắt đầu bằng 'user'
            ...
            const rawReply = parts.map(p => p.text).join('')

            const reply = rawReply || '⚠️ Không nhận được phản hồi. Vui lòng thử lại.'
