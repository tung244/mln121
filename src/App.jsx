...
import { getObjectDetail } from './data/objectDetails'
...
export default function App() {
  ...
  const [showGLBViewer, setShowGLBViewer] = useState(true) // Default to true since user asked to visualize it
  ...
  const handleObjectClick = useCallback((category) => {
    const detail = getObjectDetail(category, year)
    if (detail) {
  ...
  return (
    <>
      ...
      {loaded && (
        <div className="fixed inset-0 overflow-hidden cyber-grid" style={{ background: 'radial-gradient(ellipse at 50% 40%, #020c1f 0%, #000 100%)' }}>
          ...
          <div className="absolute inset-0 z-0">
            <Scene3D year={year} resolvedDots={resolvedDots} onDotClick={handleDotClick} isCinematic={isCinematic} onObjectClick={handleObjectClick} />
          </div>
          ...
          {/* Object Detail Modal */}
          {selectedDetail && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,10,20,0.6)', backdropFilter: 'blur(4px)' }} onClick={() => setSelectedDetail(null)}>
              <div
                className="relative max-w-md w-full p-6 rounded-2xl border border-cyan-500/50 shadow-[0_0_30px_rgba(0,255,255,0.2)]"
                style={{ background: 'linear-gradient(135deg, rgba(0,30,60,0.9) 0%, rgba(0,10,20,0.95) 100%)', animation: 'scaleIn 0.3s ease-out' }}
                onClick={(e) => e.stopPropagation()}
              >
                <button
                  onClick={() => setSelectedDetail(null)}
                  className="absolute top-4 right-4 text-cyan-400 hover:text-white hover:scale-110 transition-transform"
                >
                  ✕
                </button>
                <div className="flex items-center gap-4 mb-4">
                  <div className="text-5xl filter drop-shadow-[0_0_10px_rgba(0,255,255,0.8)] neon-pulse">
                    {selectedDetail.icon}
                  </div>
                  <div>
                    <h3 className="font-orbitron font-bold text-xl text-cyan-300 tracking-wide">{selectedDetail.title}</h3>
                    <div className="text-xs text-cyan-500 font-mono mt-1">THÔNG TIN GIAI ĐOẠN {Math.floor(year)}</div>
                  </div>
                </div>
                <div className="font-mono text-sm text-gray-300 leading-relaxed border-t border-cyan-800/50 pt-4">
                  {selectedDetail.desc}
                </div>
              </div>
            </div>
      ...
      {/* GLB Viewer Modal */}
      {showGLBViewer && (
        <GLBViewer onClose={() => setShowGLBViewer(false)} />
      )}
    </>
  )
}
